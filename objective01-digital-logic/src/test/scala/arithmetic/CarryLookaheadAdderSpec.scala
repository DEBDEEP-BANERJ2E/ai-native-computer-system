...
class CarryLookaheadAdderSpec extends AnyFlatSpec with ChiselScalatestTester {
  behavior of "CarryLookaheadAdder"

  it should "match integer addition for every 8-bit input and carry" in {
    test(new CarryLookaheadAdder(8)) { dut =>
      for {
        a <- 0 until 256
        b <- 0 until 256
        carryIn <- 0 until 2
  ...
  it should "support a 32-bit CPU datapath width" in {
    test(new CarryLookaheadAdder(32)) { dut =>

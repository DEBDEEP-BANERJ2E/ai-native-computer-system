...
2. **Objective 2: RISC-V Processor Core, System MMIO & CapabilityLite Security** (`objective02-riscv-core/`):
   ...
   - **CapabilityLite Hardware Security & Precise Trapping (Phase 8 - Frozen)**:
     - 100-bit bounded capability registers `c0`–`c7` (`tag`, `base`, `length`, `perms`, `offset`) with hardware-immutable root initialization (`c0` = NULL, `c1` = DataMemory, `c2` = SystemMMIO; write attempts discarded) and general-purpose process registers `c3`–`c7`.
     ...
     - Custom-1 (`0x2B`) capability-protected memory instructions (`CLW`, `CLH`, `CLB`, `CSW`, `CSH`, `CSB`).
